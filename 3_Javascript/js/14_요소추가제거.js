
// 추가 버튼 클릭 시 동작
document.getElementById("add").addEventListener("click", function(){

    // 요소 생성

    // div 요소 생성 (전체를 감싸는)
    const div = document.createElement("div");

    // div에 class="row" 추가
    div.classList.add("row");

    // ------------------------------------------------

    // input 요소 생성
    const input = document.createElement("input");
    
    // input에 in class 추가
    input.classList.add("in");

    // * 요소에 속성을 추가하거나 변경할 때 사용하는 함수
    // 요쇼.setAttribute("속성명", "속성값");
    // <-> 요소.removeAttribute("속성명") : 속성 제거

    // input에 type="number" 추가
    input.setAttribute("type", "number");

    // ------------------------------------------------

    // span 요소 생성 (X버튼)
    const span = document.createElement("span"); // 동적으로 요소 생성

    // span에 remove class 추가
    span.classList.add("remove");

    // span에 &times; 추가
    span.innerHTML="&times;";

    // span에 click 이벤트 동작 추가 (동적 요소에 이벤트 추가)
    span.addEventListener("click", function(){
        // alert("동적 요소에 이벤트가 추가됨");
        // 삭제 시 부모인 row가 삭제되어야 함.

        // 클릭된 X버튼의 부모 요소(div.row)를 삭제
        // 요소.remove() : 해당 요소를 삭제/제거
        this.parentElement.remove();
    })

    // ------------------------------------------------

    // 조립하기
    // div > input, span 조립
    // div.append(input);
    // div.append(span);
    div.append(input,span); // 배열 형태로도 들어갈 수 있어서 같이 한번에 적을 수 있데.

    // .container인 요소의 마지막 자식으로 추가
    document.querySelector(".container").append(div);

    input.focus(); // 화면에 추가된 input에 포커스

    
});

// 계산 버튼이 클릭 되었을 때
// input요소에 작성된 값을 모두 얻어와 합한 후 출력
document.getElementById("calc").addEventListener("click", function(){

    // 함수는 호출된 시점의 HTML 화면을 기반으로해서 해석 진행!
    const inputList = document.getElementsByClassName("in"); // 이게 밖에 있으면 추가하기 전의 것들이 안나온데...
    // console.log(inputList);

    let sum = 0;

    for(let item of inputList){
        // input이라는 요소가 담겨있겠지.
        sum += Number(item.value); // input 요소 담아서 저장
        // 이건 문자열을 + + 해서 반환해서 Number로 숫자 변환시켜줌.
        // 요소에서 읽어온 값 또는 내용은 무조건 stirng 
        // -> 필요 시 데이터 파싱 진행
    }
    alert(sum);

})

// 추가 버튼으로 생성된 X 버튼의 클래스가 "remove"이니까
// "remove" 클래스 요소를 모두 얻어와
// for문으로 하나씩 접근해서 click 이벤트 동작을 추가.

// -> 안됨...
// 처음 HTML 문서 해석 시 remove 라는 클래스를 가진 요소가 존재하지 않기 때문에
// 아래 이벤트 동작 추가 구문에 아무 요소에도 적용되지 않음.

// * 처음부터 있던 HTML 요소 == 정적 요소
// * JS로 추가되는 요소 == 동적 요소(없다가 생기는/만들어지는)

// 정적 요소 .remove에 이벤트 동작 추가한거.
// const remove = document.getElementsByClassName("remove"); 
// 애초에 html에 작성되지 않은 부분이라서 해석이 안되는거래
// 로딩이 되지마자 해석이 되고 한번 해석되면 다시는 해석을 안한데.

// for(let item of remove){
//     item.addEventListener("click", function(){
//         alert("삭제");
//     })
// }


